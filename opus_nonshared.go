...
// }
//CELTDecoder * CeltInitDecod() {
//	int err = 0;
//	OpusCustomMode *mode=NULL;
//	mode = opus_custom_mode_create(SAMPLE_RATE, FRAME_SIZE, &err);
//	if (mode == NULL)
//	{
//		printf("error create mode");
//		fflush(stdout);
//		return NULL;
//	}
//
//	CELTDecoder *celt_dec = opus_custom_decoder_create(mode, CHANNELS, &err);
//	if (err != OPUS_OK)
//	{
//		printf("error create CELTDecoder\n");
//		fflush(stdout);
//		return NULL;
//	}
//
...
func CeltCustomDecoder(reciveServerData []byte, i int) []int16 {
	pcm := make([]int16, C.FRAME_SIZE*C.CHANNELS*2)
	celt := C.CeltInitDecod()

	ret := C.opus_custom_decode(celt, ToChar(reciveServerData[i:]), (C.int)(64), (*C.opus_int16)(&pcm[0]), C.FRAME_SIZE)
	if ret < 0 {
		fmt.Printf("Err celt - %v\n", ret)
		return nil
	}
